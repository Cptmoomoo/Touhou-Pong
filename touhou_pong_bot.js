...
function create ()
{
    ...
    // Court Box
    court_box = this.add.rectangle(160, 360, 320, 240);
    this.physics.add.existing(court_box);
    ...
    this.physics.add.overlap(ball, court_box, reimuAI);
...
function reimuAI() {
    ...
    else if (ball.x > reimu.x) {
        if (Math.abs(ball.x - reimu.x) > 50) {
            AIdashRight();
        }
        else {
            AImoveRight();
